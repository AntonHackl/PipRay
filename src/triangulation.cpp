...
std::vector<Polygon_with_holes_2> repairSelfIntersectingPolygon(const Polygon_2& polygon) {
    ...
    try {
        // Method 1: Use CGAL Polygon_set_2 to repair self-intersections
        CGAL::Polygon_set_2<K> polygon_set;
        polygon_set.insert(polygon);
        
        // Extract the repaired polygons
...
{
    try {
        // Method 0: Direct CGAL triangulation
        CDT cdt;
        
        // Convert to CGAL polygon
        Polygon_2 outer_polygon = convertToPolygon2(poly.outer);
        
        // Check if the outer polygon is simple (no self-intersections)
        if (outer_polygon.is_simple()) {
            // Insert the outer boundary as constraints
            ...
            // Mark domains (inside/outside)
            mark_domains(cdt);
            
            // Extract triangles from the domain
            std::vector<Triangle> triangles = extractTriangles(cdt);
            
            if (!triangles.empty()) {
                return std::make_pair(triangles, 0); // Standard CGAL success
            }
        }
        
        // Method 1: Try polygon repair for self-intersecting polygons
        ...
        if (!repaired_polygons.empty()) {
            ...
            for (const auto& repaired_poly : repaired_polygons) {
                ...
                // Insert the repaired outer boundary
                ...
                // Insert holes from the repaired polygon
